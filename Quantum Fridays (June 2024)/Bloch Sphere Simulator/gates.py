...
# Tech Stack used: Python, Numpy

import numpy as np

# X Gate: |0〉to |1〉(x-axis rotation)
def X(intensity=100, oldx=1, oldy=0, oldz=0):
...
def Y(intensity=100, oldx=1, oldy=0, oldz=0):
...
def Z(intensity=100, oldx=1, oldy=0, oldz=0):
...
def H(intensity=100, oldx=1, oldy=0, oldz=0):
    return oldz, oldy, oldx

# S Gate: |+〉to |+i〉(z-axis rotation)
def S(intensity=100, oldx=1, oldy=0, oldz=0):
...
def antiS(intensity=100, oldx=1, oldy=0, oldz=0):
    x, y, z, theta, phi = oldx, oldy, oldz, 0, -(np.pi/2)*(intensity/100)
    x += np.sin(theta) * np.cos(phi)
    y += np.sin(theta) * np.sin(phi)
    z += np.cos(theta)
...
def T(intensity=100, oldx=1, oldy=0, oldz=0):
...
def antiT(intensity=100, oldx=1, oldy=0, oldz=0):
    x, y, z, theta, phi = oldx, oldy, oldz, 0, -(np.pi/4)*(intensity/100)
    x += np.sin(theta) * np.cos(phi)
    y += np.sin(theta) * np.sin(phi)
    z += np.cos(theta)
...
def A(theta=0, phi=0, oldx=1, oldy=0, oldz=0):
